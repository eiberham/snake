...
impl Snake {
    ...
    pub fn draw(&mut self, canvas: &mut Canvas, ctx: &mut Context) -> () {
        let sprite: Sprite<Value> = {
            let data = fs::read_to_string("./resources/sprite.json").unwrap_or_else(|error| {
        ...
        let frame: Frame<Value> =
            serde_json::from_value(sprite.frames.first().unwrap().to_owned()).unwrap();

        let size: Size<Value> = serde_json::from_value(frame.source_size.to_owned()).unwrap();
        ...
        for square in self.body.iter() {
            ...
            canvas.draw(
                &image,
                DrawParam::new()
                    .src(Rect::new(offset * 5.0, 0.0, 1.0 / 6.0, 1.0))
                    .dest([tile.x, tile.y]),
            );
        }
        ...
        canvas.draw(
            &image,
            DrawParam::new().src(src).dest([self.head.x, self.head.y]),
        );
    }
